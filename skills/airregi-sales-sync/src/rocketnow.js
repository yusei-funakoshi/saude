...
export async function fetchRocketNowSales(context, storeCfg, date) {
  ...
  try {
    ...
    const label = storeCfg.storeLabel || '';

    const result = await page.evaluate(
      ({ yymmdd, label }) => {
        // 注文行を推定収集（テーブル行 / リスト行）。最下層の行のみ採用して二重計上を防ぐ。
        const all = Array.from(document.querySelectorAll('tr, li, [class*="order"], [class*="row"]'));
        const leaf = all.filter((el) => !all.some((other) => other !== el && el.contains(other)) || el.tagName === 'TR');
        let total = 0;
        let matched = 0;
        for (const el of leaf) {
          const t = (el.innerText || '').replace(/\s+/g, ' ');
          if (!t.includes(yymmdd)) continue; // 対象日の行のみ
          if (label && !t.includes(label)) continue; // 対象店舗のみ
          if (t.includes('キャンセル')) continue; // キャンセルは除外
          const m = t.match(/([\d,]+)\s*円/g);
          if (m && m.length) {
            total += parseInt(m[m.length - 1].replace(/[^\d]/g, ''), 10) || 0;
            matched++;
          }
        }
