...
import { rmSync, readdirSync as fsReaddirSync, existsSync as fsExistsSync } from 'node:fs';
...
/** Cookie の domain がリクエスト先 host に適用されるか（host-only と親ドメインの両対応）。 */
export function cookieApplies(host, cookieDomain) {
  const d = cookieDomain.replace(/^\./, '');
  return host === d || host.endsWith('.' + d);
}

/** 復号済み cookie 配列から、指定 host 宛ての Cookie ヘッダ文字列を組み立てる。 */
export function buildCookieHeader(cookies, host) {
  const seen = new Map();
  for (const c of cookies) {
    if (!cookieApplies(host, c.domain)) continue;
    seen.set(c.name, c.value); // 後勝ち（より具体ドメインが後なら上書き）
  }
  return [...seen.entries()].map(([k, v]) => `${k}=${v}`).join('; ');
}

/** macOS の Chrome プロファイル一覧（Default / Profile N）の Cookies DB パスを返す。 */
export function listChromeProfiles() {
  const base = `${process.env.HOME}/Library/Application Support/Google/Chrome`;
  const { readdirSync, existsSync } = require_fs();
  return readdirSync(base, { withFileTypes: true })
    .filter((d) => d.isDirectory() && (d.name === 'Default' || /^Profile \d+$/.test(d.name)))
    .map((d) => `${base}/${d.name}/Cookies`)
    .filter((p) => existsSync(p));
}

function require_fs() {
  // ESM では import を上部に置くが、最小変更のため動的に解決
  return { readdirSync: fsReaddirSync, existsSync: fsExistsSync };
}
